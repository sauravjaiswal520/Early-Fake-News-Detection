...
# UPDATIONS Propogation path addition by group
    ...
    def fit(self, X_train_source_wid, X_train_source_id, X_train_user_id, X_train_ruid, y_train, y_train_cred, y_train_rucred,
        ...
        for epoch in range(1, self.config['epochs']+1):
            ...
            for i, data in enumerate(dataloader):
                with torch.no_grad():
                    X_source_wid, X_source_id, X_user_id, X_ruid, batch_y, batch_y_cred, batch_y_rucred = (item for item in data)

                self.optimizer.zero_grad()
                logit, ulogit, rulogit, r = self.forward(X_source_wid, X_source_id, X_user_id, X_ruid)

                output = model1(r,  self.initial_hidden_state[:, :r.shape[0], :]) # UPDATE CALLED 
                loss3 = self.criterion(output,  batch_y) # LOSS is calculated 
